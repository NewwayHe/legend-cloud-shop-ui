    ...
    <div v-if="!collapse" class="text-center">
        <transition name="sidebarLogoFade">
            <div class="d-flex j-center a-center m-30 overflow-h">
                <ls-image :src="shopInfo.shopAvatar" :is-preview="false" :options="{ w: '50', h: '50', br: '2' }"></ls-image>
                <div class="overflow-h flex-1 ml-10 h-50p flex-col-between text-left">
                    <div class="font-14 line-clamp2">{{ shopInfo.shopName }}</div>
                    <el-button size="mini" class="mt-10" style="width: 60px; height: 20px; padding: 3px 4px" @click.stop="goShop">查看商城</el-button>
                </div>
            </div>
        </transition>
...
export default {
    ...
    methods: {
        goShop() {
            ...
            this.$refs.dialogPreview.showDialog({
                url: this.$shareRedirectUrl + `?detailsType=shop&device=pc&shopId=${this.shopInfo.id}&isPreview=false`
            })
        }
    }
}

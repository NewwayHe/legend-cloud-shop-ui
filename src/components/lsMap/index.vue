<template>
    <div class="ls-map d-flex flex-column" :class="customClass" :style="customStyle">
		<div class="flex-start mb-10" slot="top">
			<el-input
				id="map__input"
			...
			/>
			<el-button v-if="showBtn" icon="el-icon-check" type="primary" @click="handleSubmit()" class="ml-15">确 定</el-button>
		</div>
		...
		initPoiPicker() {
			...
			window.AMapUI.loadUI(['misc/PoiPicker'], PoiPicker => {
				...
				poiPicker.on('poiPicked', function(poiResult) {
					...
					let source = poiResult.source, poi = poiResult.item;
			...
			if(this.mapSearchPlugin && this.useSearch) {
				...
				this.mapSearchPlugin.search(searchVal || this.searchAddress, (status, result) => {
					if(status == 'complete' && result.poiList.pois.length) {
						...
						const { location: { lat, lng }, pname, cityname, adname, address, name  }= result.poiList.pois[0]

...
<template>
    <section class="">
        ...
        <div>
            <el-card shadow>
                <el-card shadow :body-style="{backgroundColor:'#F5F7FA'}" class="mb-20 rounded-0" v-if="paramsList && paramsList.length">
                    <h3 class="d-inline" style="font-size:18px">优惠券退还规则(保存后生效)</h3>
                    <el-popover placement="right" width="500" trigger="hover">
                        <div class="font-12">
                            优惠券退还规则设置，勾选的规则点击保存后生效；
                            <br />1、提交订单未付款可退还，即用户提交订单却未完成付款时，订单变为失效状态时，返还订单中所有使用了的优惠券；
                            <br />2、若订单整单售后，完成退款流程时，返还订单中所有使用了的优惠券；
                            <br />3、若订单发货后，针对单核订单项进行售后，需要订单中所有订单项完成售后流程时，才返还订单中所有使用了的优惠券；
                            <br />
                        </div>
					...
					<div class="flex-start my-15">
						<el-checkbox-group v-model="couponForm.type">
						    ...
						    >{{ item.des }}</el-checkbox>
						</el-checkbox-group>
						<ls-button class="ml-30" type="primary" size="small" :asyncFunction="saveParams">保存</ls-button>
					</div>
                </el-card>
                <!-- 查询 -->
                <div class="search">
                ...
                <div class="table">
					...
					<el-table ref="multipleTable" v-loading="tableListLoading" :data="tableList" tooltip-effect="dark" class="w-100" header-row-class-name="headerRow" row-key="id" @selection-change="selectionChange">
                        ...
                        <el-table-column prop="status" label="状态" width="65">
                            <template slot-scope="scope">
                                <span
                                    :class="scope.row.status | statusTextFilter(couponStatus,0)"
                                >{{ scope.row.status | statusTextFilter(couponStatus,1) }}</span>
                            </template>
                        </el-table-column>
                            ...
                            <template slot-scope="scope">
                                <span class="table__action">
                                    <el-link
                                        :underline="false"
                                        type="primary"
                                        @click.stop="goDetail(scope.row, 'lookInfo')"
                                    >查看</el-link>
                                    <el-link 
                                        :underline="false" 
                                        type="primary" 
                                        @click.stop="goData(scope.row)">
                                    数据</el-link>
                                    <el-link
                                        :underline="false"
                                        type="primary"
                                        @click.stop="goDetail(scope.row, 'copy')"
                                    >复制</el-link>
                                    <el-link
                                    ...
                                    <el-link
                                        :underline="false"
                                        type="primary"
                                        @click.stop="viewLinks(scope.row.id)"
                                    >查看链接</el-link>
                                </span>
                            </template>
                        </el-table-column>

    ...
    <el-dialog title="选择商品" custom-class="dialog-form" width="1000px" :visible.sync="dialogVisible">
        ...
        <el-row class="mb-20">
            ...
            >
                    ...
                    <template slot-scope="scope">
                        ...
                        >
                            ...
                            <el-table-column label="规格" align="center">
                                <template slot-scope="childScope">
                                    {{ childScope.row.cnProperties || '-' }}
                                </template>
                            </el-table-column>
                            <el-table-column label="销售库存" align="center">
                                <template slot-scope="childScope">
                                    {{ childScope.row.stocks || 0 }}
                                </template>
                            </el-table-column>
                            <el-table-column label="销售价" align="center">
                                <template slot-scope="childScope">
                                    {{ childScope.row.price | priceFilter }}
                                </template>
                            </el-table-column>
                            <el-table-column align="center" label="备注">
                                <template slot-scope="childScope">
                                    {{ skuType[childScope.row.skuType] || '-' }}
                                </template>
                            </el-table-column>
                        </el-table>
                    </template>
                ...
                <el-table-column label="销售库存" align="center">
                    <template slot-scope="childScope">
                        {{ childScope.row.stocks }}
                    </template>
                </el-table-column>
                <el-table-column label="销售价" align="center">
                    <template slot-scope="childScope">
                        {{ childScope.row.price }}
                    </template>
                </el-table-column>
                <el-table-column align="center" label="备注">
                    <template slot-scope="childScope">
                        {{ childScope.row.remarks || '-' }}
                    </template>
                </el-table-column>
            </el-table>
        </el-row>
...
export default {
    props: {
        ...
        goodUrl: {
            type: [String, Number],
            default: '/activity/s/coupon/productPage'
        },
        activeGoodUrl: {
            type: [String, Number],
            default: '/activity/s/couponProduct'
        },
        activityType: {
            type: String,
            default: 'marketRewardId'
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        editCoupon(list) {
            ...
            return request.put(this.activeGoodUrl, params).then((res) => {
                if (res.code == 1) {
                    this.$emit('update:id', res.data)
                }
            })
        },
        ...
        getSpuCheckState(skuItem) {
            ...
            this.$set(skuItem, 'indeterminate', indeterminate)
            return checkedState
        },
...
::v-deep .child_table.el-table .el-table__body-wrapper tr:last-child td {
    border: 0; // 去除最后一行boder
}
</style>

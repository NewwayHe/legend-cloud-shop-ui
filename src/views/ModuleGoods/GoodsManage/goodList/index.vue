            ...
            <div class="table">
                ...
                >
                    ...
                    <el-table-column prop="status" label="状态" align="left">
                        <template slot-scope="scope">
                            <span class="status-veto" v-if="scope.row.status == -10">未发布</span>
                            <span class="status-veto" v-if="scope.row.status == 0">下线</span>
                                ...
                                <el-link :underline="false" type="primary">
                                    <el-dropdown size="medium" @command="setClick" style="line-height:19px">
                                        ...
                                        <el-dropdown-menu slot="dropdown">
                                            <el-dropdown-item key="product" :command="{ type: 'product', params: scope.row }" v-if="scope.row.draftStatus != 0">
                                                编辑商品
                                            </el-dropdown-item>
                                            
                                            <el-dropdown-item key="editDraft" :command="{ type: 'editDraft', params: scope.row }" v-if="(scope.row.draftStatus == -10 && scope.row.status != -10) || scope.row.draftStatus == -1">
                                                编辑草稿
                                            </el-dropdown-item>
                                            <el-dropdown-item key="cancelAudit" :command="{ type: 'cancelAudit', params: scope.row }" v-if="scope.row.draftStatus === 0">
                                                撤回审核
                                            </el-dropdown-item>
                                            <el-dropdown-item key="lookDraft" :command="{ type: 'lookDraft', params: scope.row }" v-if="scope.row.draftStatus === 0 && scope.row.status != -10">
                                                查看草稿
                                            </el-dropdown-item>
                                            <el-dropdown-item key="lookProduct" :command="{ type: 'lookProduct', params: scope.row }" v-if="scope.row.draftStatus === 0">
                                                查看商品
                                            </el-dropdown-item>
                                            <el-dropdown-item
                                            ...
                                            <el-dropdown-item key="draftArraignment" :command="{ type: 'draftArraignment', params: scope.row }" v-if="scope.row.draftStatus == -10">
                                                提交审核
                                            </el-dropdown-item>
                                        </el-dropdown-menu>
                                    </el-dropdown>
                                </el-link>
								<el-link :underline="false" type="primary" @click="goAddPage('copy', scope.row)">
								    复制
								</el-link>
                                <el-link v-if="scope.row.status != 1" :underline="false" type="primary" @click="deleteGood(scope.row)">
                                    删除
                                </el-link>
...
export default {
    ...
    methods: {
        ...
        productDraftRevokeArraignment(id){
            this.$confirm('撤销提审后，可重新编辑商品提交审核，确定撤销','消息确认').then(
                res=>{

		...
		<el-table v-loading="loading" :data="data.resultList" style="width: 100%">
			...
			<el-table-column label="操作" width="130">
				<template slot-scope="scope">
					<span class="text-main cursor-pointer" v-if="scope.row.fail" @click.stop="getFailPage(scope.row.id)">下载错误列表</span>
				</template>
			</el-table-column>
			<empty style="padding: 20px 0;" text="暂无导入结果" slot="empty" />
		</el-table>
		<el-row type="flex" justify="center" class="mt-30" v-if="data">
			<el-pagination :current-page.sync="data.curPageNO" :page-size="data.pageSize" layout="prev, pager, next, jumper" :total="data.total" @current-change="currentChange" />
		</el-row>

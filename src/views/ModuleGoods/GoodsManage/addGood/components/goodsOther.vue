		...
		<el-form ref="form" :model="value" :rules="rules" label-width="132px" size="small">
			...
			<el-form-item label="配送方式：" prop="deliveryType">
				<el-checkbox-group v-model="value.deliveryType">
					<el-checkbox v-for="deliveryType in deliveryTypes" :key="deliveryType.value" :label="deliveryType.value" :disabled="deliveryType.disabled">
						{{ deliveryType.label }}
					</el-checkbox>
				</el-checkbox-group>
			</el-form-item>
			...
			<el-form ref="transForm" :model="value" label-width="132px" size="small">
				<el-form-item label="运费方式：" prop="transId" :rules="transId">
					<el-button type="primary" size="small" @click="selectTrans" v-if="!isLookInfo">选择模板</el-button>
...
export default {
	...
	data() {
		...
		return {
			...
			transId: [{ required: true, message: '运费模板不能为空', trigger: 'blur' }, { validator: checkParamsList, trigger: 'blur' }],
		};
	},
	computed: {
		isLookInfo(){

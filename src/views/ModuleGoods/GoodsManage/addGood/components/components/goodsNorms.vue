		...
		<el-form ref="form" :model="model" label-width="132px" size="small" @submit.native.prevent>
			<el-form-item v-if="model.norms">
						...
						<template slot="header">
							<el-row type="flex" align="middle" justify="center">
								...
								<el-tooltip placement="right">
									...
									<div class="line-h-lg" slot="content">图片规格需选取一个图片类型的主规格</div>
								</el-tooltip>
							</el-row>
						</template>
						<template slot-scope="scope">
							<el-row type="flex" align="middle" justify="center">
								<el-radio class="line-clamp1" v-model="value.mainSpecificationId" :label="scope.row.id" style="margin-right: 5px">
									{{ scope.row.propName }}
								</el-radio>
							</el-row>
						</template>
						...
						<template slot-scope="scope">
							<el-form-item class="con-mb-0">
								<el-row type="flex" align="middle">
									...
									<el-col :span="8">
										<el-row type="flex" align="middle" v-if="!isLookInfo">
											<i
								...
								<div class="mt-20 position-relative" v-if="value.mainSpecificationId == scope.row.id && normsStyle != 'TXT'">
									<el-button
									...
									<el-form ref="forms" :model="scope.row" label-width="80px" size="small" :key="num">
										<template v-for="(item, index) in scope.row.prodPropList">
											...
											>
												<span class="line-clamp2" :title="item.name" slot="label">{{ item.name }}</span>
												<imgCenter
													v-if="!isLookInfo"
				...
				<el-row type="flex" class="mt-15">
					<el-button type="primary" plain size="small" @click="openDialogNormsTemplate" v-if="!isLookInfo">选择自定义规格</el-button>
				</el-row>
			</el-form-item>
		</el-form>
		<dialogNormsTemplate

			...
			<div class="table">
				...
				>
					...
					<el-table-column label="结算状态">
						<template slot-scope="scope">
							<div class="status-wait" v-if="scope.row.status == '1'">待确认</div>
							<div class="status-wait" v-if="scope.row.status == '2'">待审核</div>
							<div class="status-wait" v-if="scope.row.status == '3'">待结算</div>
							<div class="status-done" v-if="scope.row.status == '4'">已完成</div>
						</template>
					</el-table-column>
				...
				<el-alert type="warning" class="defalult mt-20 mb-20" :closable="false">
					<div style="line-height: 30px" class="text-999">
						...
						<p>2、账单计算公式：</p>
						<p class="ml-20">
							2.1、订单金额(含运费) - 退单金额 - 平台佣金 - 分销佣金 + 优惠券总额 - 退单优惠券金额 + 预售定金
						</p>
						<p class="ml-20">

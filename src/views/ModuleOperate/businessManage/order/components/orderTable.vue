    ...
    <div class="customTable__warp" ref="wrapTable">
                    ...
                    <table v-for="(item, indexs) in tableList" :key="item.id" class="customTable__table">
                        ...
                        <tbody>
                            <tr class="customTable__trHead">
                                <td>
                                    <el-checkbox :label="item.orderNumber" :disabled="!(item.status == 1 || (item.status == '3' && item.refundStatus == 0))" />
                                </td>
                            ...
                            <tr v-for="(goodsInfo, index) in item.orderItemDTOList" :key="goodsInfo.id">
                                        ...
                                        <div class="ml-10 flex-col-between">
                                            ...
                                            <el-link :underline="false" type="primary" @click="proPreview(goodsInfo)" style="font-size: 0;">
                                                <el-popover placement="top-start" width="500" trigger="hover" :title="goodsInfo.productName">
                                                    <div slot="reference" class="text-blue line-clamp2 text-left font-12">{{ goodsInfo.productName }}</div>
                                                    <el-link
                                                        class="text-blue"
                                                        :underline="false"
                                                        target="_blank"
                                ...
                                <!-- 交易状态 -->
                                <td v-if="!index" :rowspan="item.orderItemDTOList.length">
                                    <span :class="'1,5,10,15'.includes(item.status) ? 'status-wait' : 'status-done'">{{ item.status | getOrderStatus(item.orderType) }}</span>
                                </td>
                                ...
                                <td v-if="item.orderRefundType == 2">
                                    <div>
                                        ...
                                        <div v-else-if="goodsInfo.refundStatus == 2">{{ refundStatus[goodsInfo.refundStatus || item.refundStatus] }}</div>
                                ...
                                <td v-else-if="!index" :rowspan="item.orderItemDTOList.length">
                                    ...
                                    <span class="el-link text-blue" v-if="item.refundStatus != '0'" @click="toRefuseDetail(item,item.applyType)">查看售后</span>
                                </td>
                                    ...
                                    <div class="flex-col-center">
                                        ...
                                        <template v-if="item.status == '5' && (!item.refundStatus||item.refundStatus===3)">
                                            ...
                                            <el-link
                                                ...
                                                :underline="false"
                                                class="d-block text-blue"
                                                @click="sureStockUp(item)"
                                            >
                                                备货完毕
                                            </el-link>
                                        </template>
                                        <el-link v-if="item.status == '1' || '5'" :underline="false" class="d-block text-blue" @click="toDetail(item.id)">
                                            查看详情
                                        </el-link>
                                        <el-link v-if="!item.remarkedFlag" :underline="false" class="d-block text-blue" @click="showDialog(item, 'remark')">
                                            添加备注
                                        </el-link>
                                        <!-- 这个取消订单取消的是买家已付款，商家待发货的订单(只有待成团、待发货、已付定金才可以) -->
                                        <el-link
                                            v-if="(item.status == '5' || item.status == '2' || item.status == '3') && (item.refundStatus===0 || item.refundStatus===3)"
                                            :underline="false"
                                            class="d-block text-blue"
        ...
        <el-dialog :title="actionMap[currentAction].title" :visible.sync="orderDialog" custom-class="dialog-form-small">
                ...
                <el-form ref="form" :model="form" label-width="98px" label-position="right" size="small">
                    ...
                    <template v-if="currentAction == 'adjustOrder'">
                        ...
                        <el-form-item label="应付金额：" prop="orderAmount" :rules="[{ required: true, message: '请输入应付金额' }]">
							<lsInput v-model="form.orderAmount" :precision="2" :min="0" :max="9999999" textAlign="left"  placeholder="请输入应付金额">
								<template>元</template>
							</lsInput>
							
                        </el-form-item>
                        <el-form-item label="运费金额：" prop="freight" :rules="[{ required: true, message: '请输入运费金额' }]" v-if="currentOrder.orderType != 'SM'">
							<lsInput v-model="form.freight" :precision="2" :min="0" :max="9999999" textAlign="left"  placeholder="请输入运费金额">
								<template>元</template>
							</lsInput>
                        </el-form-item>
                        <el-form-item label="订单总额：" prop="reason">
                            ...
                            <span class="font text-nowrap" style="color: #f56b6b;margin-left: -50px;">
                                （
								<span></span>应付金额增加了￥ {{ (form.orderAmount - form.oldOrderAmount).toFixed(2) }} 
								<span v-if="currentOrder.orderType != 'SM'">，运费金额增加了￥{{ (form.freight - currentOrder.freightPrice).toFixed(2) }}</span>
                                ）
                            </span>
                        </el-form-item>
                    </template>
                    ...
                    <template v-if="currentAction == 'cancel' || currentAction == 'cancelBat' || currentAction == 'cancelPreSell'">
                        ...
                        >
                            <el-select v-model="form.reason" placeholder="请选择">
                                <el-option v-for="(item, index) in reasonList_unpaid" :id="item.id" :key="index" :label="item.value" :value="item.value" />
                            </el-select>
                        </el-form-item>
                    </template>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <ls-button v-if="currentAction != 'cancelBat'" size="small" type="primary" :asyncFunction="orderSumbit">确 定</ls-button>
                <ls-button v-else size="small" type="primary" :asyncFunction="()=>batCancelSumbit(checkList)">确 定</ls-button>
            </span>
        </el-dialog>
        ...
        orderSumbit() {
            return new Promise(async resolve=>{
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        orderApi[this.actionMap[this.currentAction].method](this.form).then((res) => {
                            if (res.code == 1) {

    ...
    <div class="customTable__warp table">
        ...
        >
            <el-table-column prop="pic" label="商品" width="280">
                <template slot-scope="scope">
                    <div class="d-flex a-center line-h-md" v-if="!scope.row.title">
            ...
            <el-table-column prop="basketCount" label="数量">
                <template slot-scope="scope">
                    ...
                    <div v-else>
                        <div v-if="scope.$index===1&&(orderInfo.preSellOrderBO.payPctType===0|| orderInfo.preSellOrderBO.payPctType===1)">
                        ...
                        <div v-else>
                            发货时间：{{scope.row.preDeliveryTime || '-'}} 开始发货
                        </div>
                    </div>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="orderStatus" label="交易状态" align="center">
                <template slot-scope="scope">
                    <span v-if="!scope.row.title" :class="'1,5,10,15'.includes(orderInfo.status) ? 'status-wait' : 'status-done'">{{ orderInfo.status | getOrderStatus(orderInfo.orderType) }}</span>
                    <div v-else>
                        <span v-if="orderInfo.preSellOrderBO.depositPayTime&&scope.$index===1">已支付</span>
                        <span v-else-if="scope.row.title==='阶段3，发货' || (orderInfo.preSellOrderBO&&orderInfo.preSellOrderBO.payPctType===0&&orderInfo.status!==-5)"></span>
            ...
            <el-table-column prop="status" label="交易操作" align="center">
                <template slot-scope="scope">
                    ...
                    <el-link class="text-blue" v-if="scope.row.refundId" :underline="false"  @click="toRefuseDetail(scope.row)">查看售后</el-link>
                    <template v-if="orderInfo.status == '5' && (!orderInfo.refundStatus||orderInfo.refundStatus===3)">
						...
						<el-link
						    ...
						    :underline="false"
						    class="d-block text-blue"
						    @click="sureStockUp(orderInfo)"
						>
						    备货完毕
						</el-link>
					</template>
                    <el-link
                        v-if="(orderInfo.status == '5' || orderInfo.status == '2' || orderInfo.status == '3') && (orderInfo.refundStatus===0 || orderInfo.refundStatus===3)"
                        :underline="false"
                        class="d-block text-blue"
        ...
        <el-dialog :title="actionMap[currentAction].title" :visible.sync="orderDialog" custom-class="dialog-form-small">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <ls-button size="small" type="primary" :asyncFunction="orderSumbit">确 定</ls-button>
            </span>
        </el-dialog>
    </div>
</template>
